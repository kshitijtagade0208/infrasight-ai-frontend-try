...
export default function ProjectDetailPage() {
  ...
  const [project, setProject] = React.useState<Project | null>(() => getProjectById(projectId) || null);
      ...
      try {
        ...
        try {
          ...
          if (projRes.ok) {
            ...
            if (Array.isArray(projData)) {
              const mapped = projData.map(mapBackendProject);
              foundProject = mapped.find(
                (p) =>
                  p.id === projectId ||
                  p.code === projectId ||
                  p.id.toLowerCase() === projectId.toLowerCase() ||
                  p.id.replace('PRJ-', '') === projectId.replace('PRJ-', '')
              ) || null;
            }
          }
        } catch (fetchErr) {
          console.error('Error fetching project from backend:', fetchErr);
        }

        if (!foundProject) {
          foundProject = getProjectById(projectId) || null;
        }
        ...
        try {
          ...
          if (alertRes.ok) {
            ...
            if (Array.isArray(alertData)) {
              projectAlerts = alertData
                .filter(
                  (a: any) =>
                    a.projectId === projectId ||
                    (foundProject && (a.projectId === foundProject.id || a.projectName === foundProject.name))
                )
                .map((a: any) => ({
                  id: a.id ? String(a.id) : `ALT-${Math.random().toString(36).substring(2, 7).toUpperCase()}`,
                  projectId: a.projectId || projectId,
                  projectName: a.projectName || foundProject?.name || '',
                  title: a.title || 'Alert Signal',
                  description: a.description || '',
                  severity: (a.severity && ['low', 'moderate', 'high', 'critical'].includes(String(a.severity).toLowerCase())
                    ? (String(a.severity).toLowerCase() as AlertSeverity)
                    : 'moderate'),
                  status: (a.status === 'acknowledged' ? 'acknowledged' : a.status === 'resolved' ? 'resolved' : 'active') as AlertStatus,
                  evidence: Array.isArray(a.evidence) ? a.evidence : ['reported', 'observed'],
                  createdAt: a.createdAt || a.created_at || new Date().toISOString(),
                  source: a.source || 'Monitoring System',
                }));
            }
          }
        } catch (alertErr) {
        ...
        if (active) {
          ...
          if (projectAlerts.length > 0) {
            ...
            const mockAlerts = getAlertsForProject(foundProject.id);
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-wrap gap-1">
                {(project.evidence ?? []).map((e: EvidenceType) => (
