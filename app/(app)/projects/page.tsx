...
export default function ProjectsPage() {
  ...
  const uniqueAgencies = React.useMemo(() => {
    return Array.from(
      new Set(projects.map((p) => p.agency).filter(Boolean) as string[])
    ).sort();
  }, []);
  ...
  const categoryCounts = React.useMemo(() => {
    return {
      ...
      high: projects.filter((p) => p.riskBand === 'high').length,
      moderate: projects.filter((p) => p.riskBand === 'moderate').length,
      low: projects.filter((p) => p.riskBand === 'low').length,
      cctvActive: projects.filter((p) => p.cctvAvailable === true).length,
    };
  }, []);
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
            <div className="flex items-center gap-2">
              ...
              <span>
                ...
                <strong className="text-foreground">{projects.length}</strong> monitored projects
              </span>
              {isFiltered && (
